...
errors cannot orphan a multi-gigabyte package. On top of that, `xctrace record`
drops multi-gigabyte `instrumentsXXXXXX.ktrace` intermediates into the per-user
temp directory even with a redirected `TMPDIR`; every `TraceSession` cleanup
sweeps the ones that appeared since its own recorder started, and killed runs'
leftovers are reclaimed by the same age-based sweep. `bench/run_benchmark.py`
never deletes scratch it does not own by default; `scripts/run_ui_benchmark.sh`
opts into that sweep of scratch abandoned by earlier interrupted runs
