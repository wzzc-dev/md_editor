...
def metric_cells(
    ...
    valid = measured(records)
    if not valid:
        count = len(SECTION_HEADERS[kind])
        return [status_text(records)] + ["n/a"] * (count - 1)
    work = work_metric(valid)
    interval = metric(valid, "frame_interval")
    if kind == "frame":
        return [fmt_number(work[0]), fmt_number(work[1]), fmt_number(interval[0]), fmt_number(interval[1]), str(dropped(valid))]
    if kind == "input":
        visible = metric(valid, "input_to_visible")
        return [fmt_number(visible[0]), fmt_number(visible[1]), fmt_number(work[0]), fmt_number(work[1])]
    if kind == "open":
        interactive = scalar_metric(valid, "first_interactive_ms")
    ...
    return [
        fmt_number(work[0]), fmt_number(work[1]), fmt_number(interval[0]), fmt_number(interval[1]), str(dropped(valid)),
...
def print_section(
    ...
    for group_label, fixtures, note in FIXTURE_GROUPS:
        ...
        for adapter in ADAPTERS:
            per_fixture = [metric_cells(grouped, adapter, fixture, scenario, kind) for fixture in fixtures]
...
def main() -> None:
    ...
    print(f"- 汇总口径：mean/P95 合并原始样本；每格 repetition `{repetition_text}`、process warm-up `{warmup_text}`；drop 为各 repetition dropped_display_frames 之和。")
    ...
    print_section("帧性能", "scroll", "frame", grouped)
    print_section("输入延迟", "input", "input", grouped)
    print_section("打开性能", "open", "open", grouped)
    print_section("滚动性能", "scroll", "scroll", grouped)
    print("## 采集口径\n")
    print("- Metric definitions：`frame_work_ms` 是适配器声明的构建/布局/绘制工作范围；`frame_interval_ms` 是该适配器报告的相邻显示/帧回调时间戳间隔。")
    print("- GPUI 的当前 hook 只有 action dispatch 计时，未覆盖 GPUI draw/paint，因此主表的 work 显示 `n/a`；其原始 dispatch 样本仍在审计 JSON/表中。")
    print("- `input_to_visible_ms`：输入动作到该适配器报告的可见帧完成；`dropped_display_frames`（Dropped display frames）：`max(ceil(interval / 16.667) - 1, 0)` 的累计值。")
    print("- `first_interactive_ms` 是首个可交互帧，不使用进程总耗时替代；打开场景没有前一帧，所以不计算 interval/drop。")
